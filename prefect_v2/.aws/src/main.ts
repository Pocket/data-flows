...
import { App, TerraformStack, CloudBackend, NamedCloudWorkspace, Fn} from "cdktf";
...
class PrefectV2 extends TerraformStack {
  ...
  constructor(scope: Construct, id: string) {
    ...
    new AwsProvider(this, "AWS", {
      ...
      assumeRole: config.testCircleCIArn === undefined ? config.testCircleCIArn : {
    ...
    this.prefectV2Secret = new DataAwsSecretsmanagerSecret(this, "prefectV2Secret", {
   ...
   this.logGroup = new CloudwatchLogGroup(this, "logGroup", {
    ...
    this.agentRoles = new AgentIamRoles(this, 'agentRoles', this.dockerSecret, this.prefectV2Secret, this.ecsCluster, caller )
  ...
  private getAgentContainerDefinition(deploymentType: string): string {
    const containerDef = buildDefinitionJSON({
      ...
      command: ["prefect", "agent", "start", "-q", `prefect-v2-queue-${config.tags.environment}-${deploymentType}`],
      secretEnvVars: [
        {
  ...
  private getAgentService(deploymentType: string) {
    const DeploymentTypeProper = deploymentType.charAt(0).toUpperCase() + deploymentType.slice(1);
    const agentTaskDef = new EcsTaskDefinition(this, `prefectV2AgentTask${DeploymentTypeProper}`, {
