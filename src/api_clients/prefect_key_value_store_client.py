...
def get_last_executed_value(flow_name: str, default_if_absent='2000-01-01 00:00:00') -> datetime:
    ...
    Args:
        ...
        - default_if_absent: The date to use as the last executed date if it is absent from the metadata, which will
          allow this flow to run the first time targeting a new Prefect Cloud env.
...
def update_last_executed_value(for_flow: str, default_if_absent='2000-01-01 00:00:00') -> None:
    """
     ...
     Args:
        ...
        - default_if_absent: The date to use as the last executed date if it isn't specified. THIS RESETS THE FLOW to
          fetch every record from the table!!
