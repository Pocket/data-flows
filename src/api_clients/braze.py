...
class UserIdentifier:
    ...
    external_id: Optional[str] # Masked Pocket user id,
    user_alias: Optional[UserAlias]
    braze_id: Optional[str] # Braze User Identifier,


@dataclass
class UserAttributes(UserIdentifier):
    """
    ...
    _update_existing_only: Optional[bool]
    ...
    # Custom Attributes
    my_custom_attribute_foobar: Optional[str]
...
@dataclass
class UserEvent(UserIdentifier):
    """
    @see https://www.braze.com/docs/api/objects_filters/event_object/
    """

    app_id: Optional[str]  # See https://www.braze.com/docs/api/api_key/#the-app-identifier-api-key
    ...
    properties: Dict[str, EventPropertyValueType]  # Key string length <= 255 characters, with no leading $ sign.
...
class Purchase(UserIdentifier):
    ...
    quantity: Optional[int]  # the quantity purchased (defaults to 1, must be <= 100 -- currently, Braze treats a quantity _X_ as _X_ separate purchases with quantity 1),
...
class UserTracking:
    ...
    events: Optional[List[UserAttributes]]
...
class BrazeClient:
    ...
    def track_users(self, user_tracking: UserTracking):
        ...
        :return:
        """
        self._post_request(user_tracking)

    def _post_request(self, data_class):
        self._session.post(data_class)
