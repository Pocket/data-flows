...
"""
The dictionary below maps subscription group names (internally defined at Pocket) 
to Braze subscription ids.
This is defined here because we don't have a separate dev/prod Dbt environment, 
so the Dbt model abstracts away from
this by using the same name (e.g. POCKET_HITS_US_DAILY) for the 
daily US Pocket Hits newsletter. 
