...
class UserAlias:
    ...
    alias_label: str
    """Value of the alias. This in combination with the alias_label must be unique for the user."""  # noqa: E501
...
class _UserIdentifier:
    ...
    external_id: Optional[str] = None  # Masked Pocket user id,
    user_alias: Union[str, UserAlias] = None  # type: ignore
    ...
    def __post_init__(self):
        ...
        assert (
            list(v is not None for v in ids).count(True) == 1
        ), f"Exactly one user ID required in {ids}"
...
class UserAttributes(_UserIdentifier):
    ...
    # (date at which the user first used the app) String in ISO 8601 format or in yyyy-MM-dd'T'HH:mm:ss:SSSZ format.  # noqa: E501
    # Also the pocket signed up at
    date_of_first_session: Optional[str] = None
    email_subscribe: Optional[str] = None

    # Braze requires that country codes be passed to Braze in the ISO-3166-1 alpha-2 standard.  # noqa: E501
    country: Optional[str] = None

    # Braze requires that language be passed to Braze in the ISO-639-1 standard.
    language: Optional[str] = None

    # Time zone name from IANA Time Zone Database (e.g., “America/New_York”). Only valid time zone values will be set.  # noqa: E501
...
EventPropertyValueType = Union[
    ...
    float,
    bool,
    str,  # length <= 255 string. Will be interpreted as a date if formatted as ISO-8601 or yyyy-MM-dd'T'HH:mm:ss:SSSZ  # noqa: E501
...
class _UserEventRequiredFields:
    ...
    time: str  # required, datetime as string in ISO 8601 or in `yyyy-MM-dd'T'HH:mm:ss:SSSZ` format  # noqa: E501
...
class UserEvent(_UserIdentifier, _UserEventRequiredFields):
    ...
    app_id: Optional[
        str
    ] = None  # See https://www.braze.com/docs/api/api_key/#the-app-identifier-api-key
...
class Purchase(_UserIdentifier, _PurchaseRequiredFields):
    ...
    # the quantity purchased (defaults to 1, must be <= 100 -- currently, Braze treats a quantity _X_ as _X_ separate  # noqa: E501
    # purchases with quantity 1),
    quantity: Optional[int] = None

    properties: Dict[str, EventPropertyValueType] = field(
        default_factory=dict
    )  # Key string length <= 255 characters, with no leading $ sign.
